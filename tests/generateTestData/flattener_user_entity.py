...
class Test_datasets:
    def dataset_one_window_10(self):
        ...
        test_df = spark.createDataFrame(
            test_data_one_window_case, schema=test_user_schema
        )
        ...
        ans_df_10 = ans_df_10.withColumn(
            "window",
    ...
    def dataset_multiple_windows_5(self):
        ...
        test_df = spark.createDataFrame(
            test_data_multiple_window_case, schema=test_user_schema
        )
        test_df = test_df.withColumn(
            "SM_TIMESTAMP", col("SM_TIMESTAMP_TEMP").cast("timestamp")
        )
    ...
    def dataset_single_window_duplicates(self):
        ...
        test_df = spark.createDataFrame(
            test_duplicate_resources_same_window, schema=test_user_schema
        )
        test_df = test_df.withColumn(
            "SM_TIMESTAMP", col("SM_TIMESTAMP_TEMP").cast("timestamp")
        )
    ...
    def dataset_check_user_based_grouping(self):
        ...
        test_df = spark.createDataFrame(
            test_user_based_grouping, schema=test_user_schema
        )
        test_df = test_df.withColumn(
            "SM_TIMESTAMP", col("SM_TIMESTAMP_TEMP").cast("timestamp")
        )
