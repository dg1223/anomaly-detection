...
from pyspark.sql.types import LongType, DoubleType, StringType, TimestampType, StructType, StructField, DateType, FloatType
...
class IPFeatureGeneratorPandas(Transformer):
    ...
    def __generateIPFeatures(self, input_df):
        ...
        ip_apps = ",".join(resources.str.extractall(r"/([a-z]{1,4})/").iloc[:, 0].unique())
        ip_avg_time_bt_records = abs(timestamps.diff().dt.total_seconds().fillna(0).mean())
        ip_max_time_bt_records = abs(timestamps.diff().dt.total_seconds().fillna(0).max())
        ip_min_time_bt_records = abs(timestamps.diff().dt.total_seconds().fillna(0).min())
        ...
        ip_count_failed = (events == 6).sum() + (events == 2).sum() + (events == 9).sum()
        ...
        ip_rep_app = ",".join(resources.str.extract(r"(rep.*?)/").iloc[:, 0].dropna().unique())
        ip_timestamp = timestamps.loc[0]

        ip_count_unique_ou = cn.str.extract(r"ou=(.*?),").iloc[:, 0].dropna().unique().size
        ip_count_unique_username = cn.str.extract(r"cn=(.*?),").iloc[:, 0].dropna().unique().size
        ip_count_unique_rep = resources.str.extract(r"rep(.*?)/").iloc[:, 0].dropna().unique().size
        ...
        ans = pd.DataFrame(data, columns=['SM_CLIENTIP',
                                          'IP_APP',
                                          'IP_AVG_TIME_BT_RECORDS',
                                          'IP_MAX_TIME_BT_RECORDS',
                                          'IP_MIN_TIME_BT_RECORDS',
